...
local function _14_()
  ...
  local config_path = fs["join-path"]({root_dir, ".nfnl.fnl"})
  local plain_dir = fs["join-path"]({root_dir, "fnl", "plain"})
  local spacey_dir = fs["join-path"]({root_dir, "fnl", "my dir"})
  local comma_dir = fs["join-path"]({root_dir, "fnl", "comma,dir"})
  fs.mkdirp(plain_dir)
  fs.mkdirp(spacey_dir)
  fs.mkdirp(comma_dir)
  core.spit(config_path, "{}")
  local function _15_()
    return assert.equals(config_path, config.find(plain_dir))
  end
  it("finds the config above a directory", _15_)
  local function _16_()
    return assert.equals(config_path, config.find(spacey_dir))
  end
  it("finds the config above a directory containing a space", _16_)
  local function _17_()
    return assert.equals(config_path, config.find(comma_dir))
  end
  it("finds the config above a directory containing a comma", _17_)
  local function _18_()
    return assert.is_nil(config.find("/some/made/up/dir"))
  end
  return it("returns nil when there's no config above the directory", _18_)
end
describe("find", _14_)
local function _19_()
  local root_dir = vim.fn.tempname()
  ...
  it("includes those files even when the cwd is inside a nested project", _26_)
  local function _27_()
    return assert.is_true(config["owner-filter"](fs["join-path"]({root_dir, "not-a-project"}))(fs["join-path"]({plain_dir, "foo.fnl"})))
  end
  it("includes files when root-dir isn't a real project root", _27_)
  local function _28_()
    local owner_3f = config["owner-filter"](root_dir)
  ...
  return it("gives the same answer for repeated calls on one directory", _28_)
end
return describe("owner-filter", _19_)
